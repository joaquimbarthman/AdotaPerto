...
import { animal, donationItem, donationItemFavorite, favorite } from "../../lib/db/schemas/index.ts";
...
favoriteRoutes.get("/items", async (c) => {
  ...
  return c.json(await db.select({ id: donationItemFavorite.id, createdAt: donationItemFavorite.createdAt, item: donationItem }).from(donationItemFavorite).innerJoin(donationItem, eq(donationItemFavorite.itemId, donationItem.id)).where(eq(donationItemFavorite.userId, currentUser.id)).orderBy(desc(donationItemFavorite.createdAt)));
});

favoriteRoutes.get("/items/check/:itemId", async (c) => {
  const currentUser = c.get("user");
  if (!currentUser) return c.json({ error: "Não autorizado" }, 401);
  const [item] = await db.select({ id: donationItemFavorite.id }).from(donationItemFavorite).where(and(eq(donationItemFavorite.userId, currentUser.id), eq(donationItemFavorite.itemId, c.req.param("itemId"))));
...
favoriteRoutes.post("/items/:itemId", async (c) => {
  ...
  const [existingItem] = await db.select({ id: donationItem.id }).from(donationItem).where(eq(donationItem.id, itemId));
  if (!existingItem) return c.json({ error: "Item não encontrado" }, 404);
  const [existing] = await db.select().from(donationItemFavorite).where(and(eq(donationItemFavorite.userId, currentUser.id), eq(donationItemFavorite.itemId, itemId)));
  if (existing) return c.json(existing);
  const [created] = await db.insert(donationItemFavorite).values({ id: crypto.randomUUID(), userId: currentUser.id, itemId }).returning();
...
favoriteRoutes.delete("/items/:itemId", async (c) => {
  const currentUser = c.get("user");
  if (!currentUser) return c.json({ error: "Não autorizado" }, 401);
  const [removed] = await db.delete(donationItemFavorite).where(and(eq(donationItemFavorite.userId, currentUser.id), eq(donationItemFavorite.itemId, c.req.param("itemId")))).returning({ id: donationItemFavorite.id });
