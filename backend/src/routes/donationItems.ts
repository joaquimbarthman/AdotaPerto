...
donationItemRoutes.get("/mine", async (c) => {
  ...
  return c.json(
    await db
      .select()
      .from(donationItem)
      .where(eq(donationItem.userId, currentUser.id))
      .orderBy(desc(donationItem.createdAt)),
  );
});
...
donationItemRoutes.get("/:id", async (c) => {
  const [item] = await db
    .select({
      ...
      itemName: donationItem.itemName,
      ...
      mainImage: donationItem.mainImage,
      images: donationItem.images,
      deliveryMethod: donationItem.deliveryMethod,
      availableUntil: donationItem.availableUntil,
      status: donationItem.status,
      userId: donationItem.userId,
      createdAt: donationItem.createdAt,
    ...
    .where(eq(donationItem.id, c.req.param("id")));
...
donationItemRoutes.post("/", async (c) => {
  ...
  if (!parsed.success)
    return c.json(
      { error: "Revise os campos da doação", details: parsed.error.flatten() },
      400,
    );

  const today = new Date().toISOString().slice(0, 10);
  if (parsed.data.availableUntil && parsed.data.availableUntil < today)
    return c.json(
      { error: "A data de disponibilidade não pode estar no passado" },
      400,
    );
...
donationItemRoutes.put("/:id", async (c) => {
  ...
  const [existing] = await db
    .select()
    .from(donationItem)
    .where(eq(donationItem.id, id));
  if (!existing) return c.json({ error: "Item não encontrado" }, 404);
  if (existing.userId !== currentUser.id)
    return c.json({ error: "Sem permissão" }, 403);

  const updateSchema = itemSchema
    .partial()
    .extend({ status: z.enum(["Disponível", "Pausado", "Doado"]).optional() });
