...
mapRoutes.get("/listings", async (c) => {
  try {
    ...
    console.error("[map/listings] Falha ao consultar anúncios", {
      code,
      action: schemaMissing ? "Confira DATABASE_URL e inicialize/atualize as tabelas do banco." : "Confira a conexão e os logs do banco.",
    });
    return c.json({
      error: schemaMissing
      ...
      code: schemaMissing ? "MAP_DATABASE_NOT_READY" : "MAP_LISTINGS_UNAVAILABLE",
    }, 503);
  }
});
